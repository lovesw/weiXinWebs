...
import string
import time
from blog import ierror


def getSHA1(token, timestamp, nonce):
    """用SHA1算法生成安全签名
    @param token:  票据
    @param timestamp: 时间戳
    @param nonce: 随机字符串
...
class PKCS7Encoder():
    ...
    def encode(self, text):
        ...
        return text + (pad * amount_to_pad).encode(encoding="utf-8")
...
class Prpcrypt(object):
    ...
    def encrypt(self, text, appid):
        ...
        """
        # 16位随机字符串添加到明文开头
        temp = struct.pack("I", socket.htonl(len(text)))
        temp += (text + appid).encode("utf-8")
        text = self.get_random_str().encode(encoding="utf-8") + temp
        # 使用自定义的填充方式对明文进行补位填充
        pkcs7 = PKCS7Encoder()
